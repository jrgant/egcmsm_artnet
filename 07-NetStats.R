...
calc_nodefactor <- function(outcome, groupvar, data = attr_dt) {
  ...
  dt[, target := mean * N]
  targ_stats <- dt[, target]
  names(targ_stats) <- dt[, get(groupvar)]
  targ_stats
}
...
################################################################################
                        ## NODEMIX HELPER OBJECTS ##
################################################################################

## Assigns numeric value to each race/ethnicity category.
fix_race_names <- function(names_vec) {
  names_vec <- gsub("b", 1, names_vec) # Black
  names_vec <- gsub("h", 2, names_vec) # Hispanic
  names_vec <- gsub("o", 3, names_vec) # Other
  names_vec <- gsub("w", 4, names_vec) # White
  names_vec
}

## Inserts a period between two category identifiers (used in ergm term names)
fix_combo_names <- function(names_vec) {
  gsub("([a-z1-5])([a-z1-5])", "\\1.\\2", names_vec)
}
...
names(nodemix_race.eth_main) <- fix_combo_names(fix_race_names(
  names(nodemix_race.eth_main)
))
...
names(nodemix_age.grp_main) <- fix_combo_names(
  names(nodemix_age.grp_main)
)
...
## average duration by age group combo (within partnership)
...
durat_wks_main_byagec <- unname(rowMeans(sapply(
  pdat$main$durat_wks_byagecombo$analyses,
  function(.x) predict(.x, newdata = pred_main_durat_byagec, type = "response")
)))
names(durat_wks_main_byagec) <- fix_combo_names(
  pred_main_durat_byagec$age_combo
)
...
names(nodemix_race.eth_casl) <- fix_combo_names(fix_race_names(
  names(nodemix_race.eth_casl)
))
...
names(nodemix_age.grp_casl) <- fix_combo_names(
  names(nodemix_age.grp_casl)
)
...
durat_wks_casl_byagec <- unname(rowMeans(sapply(
  pdat$casl$durat_wks_byagecombo$analyses,
  function(.x) predict(.x, newdata = pred_casl_durat_byagec, type = "response")
)))
names(durat_wks_casl_byagec) <- fix_combo_names(
  pred_casl_durat_byagec$age_combo
)
